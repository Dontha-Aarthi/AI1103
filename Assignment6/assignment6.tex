...
\end{center}
    ...
    \end{enumerate}
\section{Solution}
Let, $X=\{0,1,2\}$ be a random variable representing the 3 coins:
\begin{enumerate}
    \item $X=0$ : Coin with heads on both faces\\
    \item $X=1$ : Coin with tails on both faces\\
    \item $X=2$ : Coin with head on one face and tail on another face
\end{enumerate}
Probability for choosing a coin is
\begin{align}
    \pr{X=0}=\pr{X=1}=\pr{X=2}=\frac{1}{3}
\end{align}
Let, $Y=\{0,1\}$ be a random variable representing the outcomes of the coin, 0 for heads and 1 for tails.
\begin{align}
